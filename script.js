...
window.runCCode=async()=>{let out=document.querySelector('#codeOutput'),code=document.querySelector('#editor').value.trim();if(!code){out.textContent='Please enter C code.';return;}if(!/\#include\s*<stdio\.h>/.test(code)){out.textContent='Only C programs are supported. Include <stdio.h> in your program.';return;}out.textContent='Compiling and running C code…';try{let r=await fetch('https://wandbox.org/api/compile.json',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({compiler:'gcc-head',code,stdin:'',options:[],compiler_option_raw:'-std=c11 -O0 -Wall'})});let j=await r.json();let text=(j.program_output||'')+(j.program_error?('\n'+j.program_error):'')+(j.compiler_error?('\n'+j.compiler_error):'');out.textContent=text.trim()||'Program executed successfully with no output.';}catch(e){out.textContent='Unable to reach the C compiler service. Check your internet connection and try again. Error: '+e.message;}}
...
return pageIntro('Practice Zone','Complete the test to record your Practice Zone progress. The enlarged question bank covers all major DSA topics.')+`<div class="stats-grid"><article class="stat-card"><span>Question Bank</span><strong>${qs.length}</strong><small>MCQs covering major DSA concepts</small></article><article class="stat-card"><span>Test Completion</span><strong id="practiceStatus">${localStorage.getItem('practiceResult')?'Completed':'Ready'}</strong><small>Updated after test submission</small></article><article class="stat-card"><span>Recommended Score</span><strong>60%</strong><small>Target for strong preparation</small></article><article class="stat-card"><span>Progress</span><strong>Combined</strong><small>Shown in Student Progress</small></article></div><div id="quizWrap">${qs.map((q,i)=>`<div class="quiz-card"><h4>${i+1}. ${q[0]}</h4>${q[1].map((o,j)=>`<label class="option"><input type="radio" name="q${i}" value="${j}"> ${o}</label>`).join('')}</div>`).join('')}</div><button class="primary-btn" onclick='scoreQuiz(${JSON.stringify(qs.map(q=>q[2]))})'>Submit Test & Update Progress</button><div id="quizResult"></div><div class="panel" style="margin-top:16px"><h3>Additional Practice Tasks</h3><p><b>Fill in the blank:</b> A ______ is used internally by recursive function calls. <b>True/False:</b> A queue follows LIFO. <b>Output-based:</b> Trace push(10), push(20), pop(), peek(). <b>Coding:</b> Implement insertion at a given position in a singly linked list.</p></div>`+markBtn('practice');
}
window.scoreQuiz=ans=>{
let s=0, unanswered=0;
ans.forEach((a,i)=>{let x=document.querySelector('input[name="q'+i+'"]:checked');if(!x)unanswered++;else if(+x.value===a)s++});
let pct=Math.round(s/ans.length*100);
let done=JSON.parse(localStorage.getItem('dsaDone')||'[]');
if(!done.includes('practice'))done.push('practice');
localStorage.setItem('dsaDone',JSON.stringify(done));
localStorage.setItem('practiceResult',JSON.stringify({score:s,total:ans.length,pct,unanswered,submittedAt:new Date().toISOString()}));
render('practice');
setTimeout(()=>{
const status=document.querySelector('#practiceStatus');
if(status)status.textContent='Completed';
const result=document.querySelector('#quizResult');
if(result)result.innerHTML='<div class="result-box"><b>Score: '+s+'/'+ans.length+'</b> ('+pct+'%) — Test submitted successfully. '+(unanswered?'You left '+unanswered+' question(s) unanswered. ':'')+'Practice Zone is now marked complete and included in Student Progress.</div>';
},0);
};
